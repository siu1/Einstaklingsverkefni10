...
export function load() {
  const ourImages = localStorage.getItem(LOCALSTORAGE_KEY);
  return JSON.parse(ourImages);
}
...
export function save(type, mediaUrl, text, title) {
  ...
  if (!storedImages.some((obj) => obj.mediaUrl === mediaUrl)) {
    storedImages.push({
      type,
      mediaUrl,
      text,
    ...
    const ourJSON = JSON.stringify(storedImages);
